...
export default function AddEditor() {
  ...
  const onSubmit = async (data) => {
    ...
    else toast.error("Failed to add editor");
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="m-4 space-y-4 max-w-xl">
        <div>
          <label className="text-sm">Editor ID (unique)</label>
          <input {...register("editorId", { required: true })} className="border p-2 w-full mt-1 focus:outline-none focus:border-primary" placeholder="e.g. ED001" />
        </div>
